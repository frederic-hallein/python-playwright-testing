"""
Module for the InventoryPage class, which handles
login actions and assertions for the inventory page.
"""
...
class InventoryPage(BasePage):
    ...
    def sort_inventory_by(self, sort: str) -> None:
        """Sort inventory items by the specified sort option."""
        if sort not in self._valid_sort_options.values():
            raise AssertionError(f"Sorting by '{sort}' is not a valid option.")

        product_sort_dropdown = self.get_page_locator_by_selector(
            InventoryPageLocators.product_sort_dropdown)
    ...
    def get_inventory_item_name(self, inventory_item_page_locator: Locator) -> str:
        """Return the name of the given inventory item."""
        item_name = self.get_text_content(inventory_item_page_locator
            .filter(has=self.get_page_locator_by_selector(
                InventoryPageLocators.inventory_item_name))
        )
        return item_name

    # ASSERT -------------------------------------
    def expect_inventory_page_to_be_visible(self) -> None:
        """Assert that all main elements of the inventory page are visible."""
        shopping_cart_button = self.get_page_locator_by_id(
            InventoryPageLocators.shopping_cart_button)
        product_sort_dropdown = self.get_page_locator_by_selector(
            InventoryPageLocators.product_sort_dropdown)
    ...
    def expect_inventory_to_be_sorted_by(self, sort: str) -> None:
        """Assert that inventory items are sorted by the specified option."""
